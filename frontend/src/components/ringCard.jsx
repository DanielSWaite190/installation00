...
function RingCard() {
  ...
  return (
    <React.Fragment>
      {rings.map((halo) => (
        <Card style={{with: '18rem'}} key={halo.id}>
          <Card.Body >
            <Card.Title>Installation {halo.installation}</Card.Title>
            <Card.Img variant="top" src={halo.img ? halo.img : dimage}/>
              <p>Rang{halo.rang}</p>
              <p>Array{halo.array}</p>
              <p>Anchor{halo.anchor}</p>
              <p>Operational{halo.operational}</p>
            </Card.Body>
        </Card>
        ))}
    </React.Fragment>
